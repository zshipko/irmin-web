...
module Make (Store : Irmin_graphql.STORE) = struct
  ...
  let handle ssl handler server =
    let server =
      match ssl with
      | Some (`Certificate crt, `Key key) ->
        Yurt.Server.configure_tls server crt key
      | None ->
        server
    in
    match handler with
  ...
  let run_custom ?handler ?ssl ?(addr = "localhost") ?(port = 8080)
      ?(static = "./") t =
  ...
  let run ?handler ?ssl ?(addr = "localhost") ?(port = 8080) ~title ~css ~js
      ~html t =
    let html' = html in
    let open Yurt.Server in
    let graphql_port = port + 1 in
