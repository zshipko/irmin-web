...
let run_simple ?print_info:(pi = true) name ~css ~js ~html =
  let run address port
    ...
    let p =
      ...
      >>= fun () -> Server.run_simple ~addr:address ~css ~js ~html ~port server
    in
    Lwt_main.run p
  in
  let main_t =
    Term.(const run $ address $ port $ Irmin_unix.Resolver.store $ mutations)
  in
...
let run ?print_info:(pi = true) name =
  let run address port
    ...
    let p =
      ...
      >>= fun () -> Server.run ~addr:address ~static ~port server
    in
    Lwt_main.run p
  in
  let main_t =
